...
object LegalCopy {
    const val AGREEMENT_VERSION = "v3"
    const val UPDATED_AT = "2026年6月"
    ...
    const val SERVICE_NATURE_ACK =
        "我理解：内容访问码仅开通本 App 内参考内容访问（可能含有效期限），" +
        "不会自动修改游戏或提升帧率；到期后需另行续费"

    const val PLAN_PICKER_HINT =
        "购卡后请使用与所选规格一致的访问码；规格不符可能无法激活或导致权益时长不符"

    const val PURCHASE_CONFIRM_TITLE = "购卡须知"

    fun purchaseConfirmContent(selectedPlanTitle: String? = null): String {
        val planLine = selectedPlanTitle?.trim()?.takeIf { it.isNotEmpty() }?.let { plan ->
            "· 您当前选择：$plan，请确认与购买页商品规格、价格及时长一致\n"
        } ?: "· 请按页面所选规格（月/季/年/永久）确认购卡类型\n"

        return "您将购买的是「本 App 内参考内容访问码」，不是游戏内自动改帧或性能提升服务。\n\n" +
            planLine +
            "· 月/季/年卡为限时访问权限，到期后需购买并激活新的访问码方可继续使用；永久卡长期有效（以产品说明为准）\n" +
            "· 具体价格、时长以购买页展示为准；本 App 不代收代付，亦不提供应用内自动续费\n" +
            "· 方案需您在游戏中自行手动设置\n" +
            "· 实际帧率由手机芯片、散热、游戏版本、网络等因素决定，本 App 无法保证达到所选帧率\n" +
            "· 访问码一经使用通常不可退换，请妥善保管\n" +
            "· 购买链接将在浏览器打开，请认准官方渠道，谨防仿站\n" +
            "· 付完款后请返回本 App 输入访问码完成激活；可关注微信公众号「画质怪兽」获取教程与售后帮助\n" +
            "· 未满 18 周岁请在监护人同意下购买\n\n" +
    ...
    const val CARD_ACTIVATE_CONFIRM_CONTENT =
        ...
        "· 不会自动修改游戏设置或提升帧率\n" +
        "· 请在游戏内对照方案手动调整\n" +
        "· 每张访问码通常仅可使用一次，且可能对应固定时长规格（月/季/年/永久）\n" +
        "· 请确保访问码规格与您购买时选择的一致；限时卡到期后相关功能将停止，需续费后重新激活\n\n" +
    ...
    val FIRST_LAUNCH_SUMMARY_BULLETS = listOf(
        ...
        "「内容访问码」仅用于解锁 App 内参考内容访问（可能含月/季/年等有效期限），不是游戏内改帧或性能提升服务。",
        "限时访问到期后需另行购买并激活新的访问码；永久规格以产品内说明为准。",
    ...
    private val userAgreement = listOf(
        ...
        LegalSection(
            ...
            paragraphs = listOf(
                ...
                "付费访问码可能对应不同有效期限（如月卡、季卡、年卡、永久等），具体以购买页及访问码说明为准；限时权限到期后，未续费前相关付费功能将停止提供。",
                "同一设备在限时权限有效期内续费或更换访问码的，权益时长按产品规则叠加或更新，以系统记录为准。",
    ...
    private val privacyPolicy = listOf(
        ...
        LegalSection(
            heading = "二、信息的使用",
            paragraphs = listOf(
                "我们使用上述信息用于：提供方案查询与推荐、激活状态验证（含访问权限有效期限）、改进产品体验、处理您的反馈与售后。",
    ...
    private val paymentTerms = listOf(
        ...
        LegalSection(
            heading = "二、时长规格与有效期限",
            paragraphs = listOf(
                "内容访问码可能分为不同规格，例如月卡、季卡、年卡、永久等；各规格的有效期限、价格以购买页及本应用内展示为准。",
                "限时规格自成功激活之日起计算有效期限（具体起算规则以系统记录为准），到期后未续费的，相应付费访问权限终止。",
                "永久规格在访问码成功激活后，在运营方持续提供服务且未违反本说明的前提下长期有效；运营方因业务调整终止服务的，将依法处理未履行部分。",
                "续费通常需购买新的访问码并重新激活；本应用不提供应用内自动扣费或自动续费功能。",
                "请确保购买的访问码规格与您在应用内选择的规格一致，规格不符可能导致无法激活或权益时长与预期不符。",
            ),
        ),
        LegalSection(
            heading = "三、免费与付费的区别",
        ...
        LegalSection(
            ...
            paragraphs = listOf(
                ...
                "购买前请在应用内确认所需规格（月/季/年/永久等），购买后请在本应用「内容访问」页面输入与规格匹配的访问码。",
                "每张访问码通常仅可使用一次，绑定设备后请妥善保管。",
                "若激活失败，请检查访问码是否输入正确、是否已被使用、规格是否匹配，并通过「意见反馈 - 激活/购卡问题」联系我们。",
            ),
        ),
        LegalSection(
            heading = "五、退款与争议",
            paragraphs = listOf(
                ...
                "限时访问码已激活但权益时长尚未届满的，除法律另有规定外，一般不支持按剩余天数折算退款。",
                "若因系统故障导致重复扣费或未到账访问码，请保留购买凭证并通过意见反馈联系我们，我们将核实后协助处理。",
                "请理性消费。本产品不承诺游戏内效果，因对服务性质、规格或有效期限理解偏差导致的争议，请结合购买前提示与本文说明协商解决。",
            ),
        ),
        LegalSection(
            heading = "六、售后联系",
            paragraphs = listOf(
                ...
                "请尽量提供：购买时间、访问码后四位（勿公开完整访问码）、所选规格、问题描述与截图。",
                "公众号：画质怪兽（hzgsapp）",
            ),
        ),
        LegalSection(
            ...
            paragraphs = listOf(
                ...
                "应用内展示的「剩余天数」等信息由服务端根据激活记录计算，仅供参考；到期判定以服务端校验结果为准。",
            ),
        ),
    )
}
