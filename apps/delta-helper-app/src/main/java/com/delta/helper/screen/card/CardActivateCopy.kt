package com.delta.helper.screen.card

import com.delta.helper.screen.legal.LegalCopy

object CardActivateCopy {
    const val PAGE_TITLE = "内容访问"
    const val PAGE_SUBTITLE = "CONTENT ACCESS"
    const val PAGE_DESC = "选择规格购买访问码，或输入已有访问码开通"
    const val HERO_BADGE = "SECURE · ONE-TIME KEY"
    const val PRODUCT_NATURE_SHORT = LegalCopy.PRODUCT_NATURE_SHORT
    const val INPUT_PLACEHOLDER = "请输入内容访问码"
    const val TAB_PURCHASE = "购买访问码"
    const val TAB_ACTIVATE = "已有访问码"
    const val PLAN_PICKER_TITLE = "选择规格"
    const val PLAN_PICKER_HINT = "购后请使用与所选规格一致的访问码激活"
    const val PURCHASE_LEGAL_HINT = "点击购买将先展示《购卡须知》，确认后在浏览器完成支付；本 App 不代收代付"
    const val PURCHASE_PAYMENT_DOC_PREFIX = "购前可阅读"
    const val PURCHASE_PAYMENT_DOC_LINK = "《付费与激活说明》"
    const val ACTIVATE_BUTTON = "确认激活"
    const val ACTIVATING_BUTTON = "激活中..."
    const val PURCHASE_BUTTON = "去购买"
    const val NO_PURCHASE_LINK = "购卡链接由授权渠道提供，请通过购买页获取内容访问码"
    const val SERVICE_NATURE_ACK = LegalCopy.SERVICE_NATURE_ACK
    const val TIPS_TEXT =
        "1. 内容访问码用于开通本 App 内参考内容访问权限，不会自动修改游戏。\n" +
        "2. 选择规格后点击购卡按钮，在浏览器完成购买；具体价格以购买页为准。\n" +
        "3. 购买后请将访问码粘贴至「已有访问码」并完成激活。\n" +
        "4. 月/季/年卡为限时访问，到期后需购买新访问码续费；本 App 不提供自动续费。\n" +
        "5. 每张访问码通常仅可使用一次，请妥善保管。\n" +
        "6. 实际帧率取决于设备与游戏内设置，本 App 不作效果保证。\n" +
        "7. 售后问题请通过购买渠道或官方客服联系我们。"
    const val TIPS_TITLE = "使用说明"
}
